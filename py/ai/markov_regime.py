...
def fetch(symbol: str, timeframe:str,limit = 999999):
    ...
    if timeframe in ['10s','1m','1d']:
            # sono nel DB
            df = get_df(query,  params= (symbol, timeframe, limit))
            df = df.iloc[::-1].reset_index(drop=True)
    else:
            # li prendo dalle candele di 1m
            df = get_df(query,  params= (symbol, "1m", limit))
...
def walk_forward_backtest(
    ...
    lab = np.asarray(labels, dtype=int) # stato
    rets = daily_returns.to_numpy(dtype=float) # gain
    ...
    for k, t in enumerate(range(min_train, len(lab) - 1)):
        ...
        current_state = lab[t]
        #print(current_state,P_t[current_state, 2] ,P_t[current_state, 0])
        signal = float(P_t[current_state, 2] - P_t[current_state, 0])
        #print(signal)

        position = float(np.sign(signal))
        gain_sign = float(np.sign(rets[t + 1]))
        if position == gain_sign:
            cc+=1

        strategy_returns[k] = position * rets[t + 1]

#        print(position,gain_sign,rets[t + 1])
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.ticker:
            ...
            #print(type(close))
            close = fetch(args.ticker,timeframe="1d")
            print(close)
           # exit(0)
