 ...
 */
//function ensureNever(_value: never) { }
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 155.89"><g><circle cx="220.68" cy="77.94" r="32.86" style="fill: var(--fill);"/><path d="M138.84,77.94c0,12.01-3.69,23.15-10,32.37h42.32v-.22c-6.21-9.17-9.84-20.24-9.84-32.15s3.63-22.97,9.84-32.15v-.22h-42.32c6.3,9.22,10,20.36,10,32.37Z" style="fill: var(--fill);"/><path d="M222.06,0H77.94C34.9,0,0,34.9,0,77.94h0c0,43.05,34.9,77.94,77.94,77.94h144.11c43.05,0,77.94-34.9,77.94-77.94h0C300,34.9,265.1,0,222.06,0ZM279.4,77.94c0,31.75-25.74,57.49-57.49,57.49H78.09c-31.75,0-57.49-25.74-57.49-57.49h0c0-31.75,25.74-57.49,57.49-57.49h143.82c31.75,0,57.49,25.74,57.49,57.49h0Z" style="fill: var(--fill);"/><circle cx="79.33" cy="77.94" r="32.86" style="fill: var(--fill);"/></g></svg>`;
const css = `a#pipsend-attr-logo{--fill:#13d3c9;position:absolute;left:10px;bottom:10px;height:24px;width:46px;margin:0;padding:0;border:0;z-index:3;opacity:0.8;transition:opacity 0.3s;}a#pipsend-attr-logo:hover{opacity:1;}a#pipsend-attr-logo[data-dark]{--fill:#13d3c9;}`;
...
class AttributionLogoWidget {
    ...
    _private__render() {
        ...
        if (this._private__visible) {
            ...
            this._private__element.href = `https://pipsend.com${this._private__getUTMSource()}`;
            this._private__element.title = 'Powered by Pipsend Charts';
            this._private__element.id = 'pipsend-attr-logo';
 ...
 * @param options - SMA configuration options
 * @returns The SMA line series
 */
function applySMA(series, chart, options = {}) {
    const period = options.period ?? 20;
    const color = options.color ?? '#2196F3';
    // Create the SMA line series as overlay (paneIndex: 0)
...
/**
 * Apply EMA (Exponential Moving Average) indicator to a series
 * @param series - Source series to calculate EMA from
 * @param chart - Chart instance
 * @param options - EMA configuration options
 * @returns The EMA line series
 */
function applyEMA(series, chart, options = {}) {
    const period = options.period ?? 12;
    const color = options.color ?? '#FF9800';
    // Create the EMA line series as overlay (paneIndex: 0)
    ...
    const calculateEMA = (data) => {
        ...
        ema = ema / period;
        if (data[period - 1] && data[period - 1].time) {
            results.push({
...
/**
 * Apply WMA (Weighted Moving Average) indicator to a series
 * @param series - Source series to calculate WMA from
 * @param chart - Chart instance
 * @param options - WMA configuration options
 * @returns The WMA line series
 */
function applyWMA(series, chart, options = {}) {
    const period = options.period ?? 20;
    const color = options.color ?? '#4CAF50';
    // Create the WMA line series as overlay (paneIndex: 0)
    ...
    const updateWMA = () => {
        try {
            ...
            const wmaData = calculateWMA([...seriesData]);
            if (wmaData && wmaData.length > 0) {
    ...
    // Subscribe to data changes
    series.subscribeDataChanged(updateWMA);
    // Initial calculation
    updateWMA();
    return wmaSeries;
}
...
/**
 * Apply Bollinger Bands indicator to a series
 * @param series - Source series
 * @param chart - Chart instance
 * @param options - Bollinger Bands configuration
 * @returns Object with upper, middle, and lower band series
 */
function applyBollingerBands(series, chart, options = {}) {
    const period = options.period ?? 20;
    const stdDev = options.stdDev ?? 2;
    // Create three line series as overlays
    const upperSeries = chart.addSeries(lineSeries, {
        color: options.colorUpper ?? '#3F51B5',
        lineWidth: 1,
    }, 0);
    const middleSeries = chart.addSeries(lineSeries, {
        color: options.colorMiddle ?? '#2196F3',
        lineWidth: 2,
    }, 0);
    const lowerSeries = chart.addSeries(lineSeries, {
        color: options.colorLower ?? '#3F51B5',
        lineWidth: 1,
    }, 0);
    // Function to calculate Bollinger Bands
    const calculateBollingerBands = (data) => {
        if (!data || data.length < period) {
            return { _internal_upper: [], _internal_middle: [], _internal_lower: [] };
        }
        const prices = [];
        for (const d of data) {
            if ('close' in d) {
        ...
        for (let i = period - 1; i < prices.length; i++) {
            ...
            if (data[i] && data[i].time) {
                ...
                upperBand.push({ time, value: upper });
                middleBand.push({ time, value: sma });
                lowerBand.push({ time, value: lower });
            }
        }
        return { _internal_upper: upperBand, _internal_middle: middleBand, _internal_lower: lowerBand };
    };
    // Update function
    const updateBollingerBands = () => {
        try {
            ...
            const bands = calculateBollingerBands([...seriesData]);
            if (bands._internal_upper.length > 0) {
                upperSeries.setData(bands._internal_upper);
                middleSeries.setData(bands._internal_middle);
                lowerSeries.setData(bands._internal_lower);
            }
        }
        catch (error) {
    ...
    // Subscribe to data changes
    series.subscribeDataChanged(updateBollingerBands);
    // Initial calculation
...
/**
 * Apply RSI indicator to a series
 * @param series - Source series to calculate RSI from
 * @param chart - Chart instance to add the indicator to
 * @param options - RSI configuration options
 * @returns The RSI line series
 */
function applyRSI(series, chart, options = {}) {
    const period = options.period ?? 14;
    const color = options.color ?? '#9C27B0';
    // Create the RSI line series in a separate pane (paneIndex: 1)
    // This creates a new panel below the main chart, like in TradingView
    ...
    }, 1); // paneIndex: 1 creates a new pane
    ...
    rsiSeries.priceScale().applyOptions({
        ...
        borderVisible: false,
        // Set fixed min/max for RSI range
        autoScale: false,
    });
    ...
    const calculateRSI = (data) => {
        ...
        // Push first result
        if (data[period] && data[period].time) {
            results.push({
    ...
    const updateRSI = () => {
        try {
            ...
            const rsiData = calculateRSI([...seriesData]);
    ...
    // Subscribe to data changes
    series.subscribeDataChanged(updateRSI);
    // Initial calculation
    updateRSI();
    return rsiSeries;
}
...
/**
 * Apply MACD indicator to a series
 * @param series - Source series
 * @param chart - Chart instance
 * @param options - MACD configuration
 * @returns Object with macd, signal, and histogram series
 */
function applyMACD(series, chart, options = {}) {
    const fastPeriod = options.fastPeriod ?? 12;
    const slowPeriod = options.slowPeriod ?? 26;
    const signalPeriod = options.signalPeriod ?? 9;
    // Create series in separate pane (paneIndex: 2 for MACD)
    const macdSeries = chart.addSeries(lineSeries, {
        color: options.colorMACD ?? '#2196F3',
        lineWidth: 2,
    }, 2);
    const signalSeries = chart.addSeries(lineSeries, {
        color: options.colorSignal ?? '#FF6D00',
        lineWidth: 2,
    }, 2);
    const histogramSeries$1 = chart.addSeries(histogramSeries, {
        color: options.colorHistogram ?? '#26a69a',
    }, 2);
    // Calculate EMA
    ...
    const calculateMACD = (data) => {
        if (!data || data.length < slowPeriod + signalPeriod) {
            return { _internal_macd: [], _internal_signal: [], _internal_histogram: [] };
        }
        const prices = [];
        for (const d of data) {
            if ('close' in d) {
        ...
        return { _internal_macd: macdData, _internal_signal: signalData, _internal_histogram: histogramData };
    };
    // Update function
    const updateMACD = () => {
        try {
            ...
            const result = calculateMACD([...seriesData]);
            if (result._internal_macd.length > 0) {
                macdSeries.setData(result._internal_macd);
                signalSeries.setData(result._internal_signal);
                histogramSeries$1.setData(result._internal_histogram);
            }
        }
        catch (error) {
    ...
    // Subscribe to data changes
    series.subscribeDataChanged(updateMACD);
    // Initial calculation
    updateMACD();
    return { macd: macdSeries, signal: signalSeries, histogram: histogramSeries$1 };
}

/**
 * Apply Stochastic Oscillator indicator to a series
 * @param series - Source series
 * @param chart - Chart instance
 * @param options - Stochastic configuration
 * @returns Object with %K and %D line series
 */
function applyStochastic(series, chart, options = {}) {
    const kPeriod = options.kPeriod ?? 14;
    const dPeriod = options.dPeriod ?? 3;
    // Create %K line (fast)
    const kSeries = chart.addSeries(lineSeries, {
        color: options.colorK ?? '#2196F3',
        lineWidth: 2,
    }, 4);
    // Create %D line (slow - signal)
    const dSeries = chart.addSeries(lineSeries, {
        color: options.colorD ?? '#FF6D00',
        lineWidth: 2,
    }, 4);
    // Configure price scale (0-100)
    ...
    const calculateStochastic = (data) => {
        if (!data || data.length < kPeriod) {
            return { _internal_k: [], _internal_d: [] };
        }
        const kValues = [];
        // Calculate %K
        ...
        return { _internal_k: kData, _internal_d: dData };
    };
    // Update function
    const updateStochastic = () => {
        try {
            const seriesData = series.data();
            if (!seriesData || seriesData.length === 0) {
                return;
            }
            const result = calculateStochastic([...seriesData]);
            if (result._internal_k.length > 0) {
                kSeries.setData(result._internal_k);
                dSeries.setData(result._internal_d);
            }
        }
        catch (error) {
    ...
    // Subscribe to data changes
    series.subscribeDataChanged(updateStochastic);
    // Initial calculation
    updateStochastic();
...
/**
 * Apply ATR (Average True Range) indicator to a series
 * @param series - Source series
 * @param chart - Chart instance
 * @param options - ATR configuration
 * @returns The ATR line series
 */
function applyATR(series, chart, options = {}) {
    const period = options.period ?? 14;
    const color = options.color ?? '#FF9800';
    // Create ATR line series in separate pane (paneIndex: 6)
    ...
    const calculateATR = (data) => {
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            if ('high' in current && 'low' in current && 'close' in previous) {
    ...
    const updateATR = () => {
        try {
            ...
            const atrData = calculateATR([...seriesData]);
    ...
    // Subscribe to data changes
    series.subscribeDataChanged(updateATR);
    // Initial calculation
    updateATR();
    return atrSeries;
}
 ...
 * @param series - Source series to sync with
 * @param chart - Chart instance
 * @param options - Volume configuration
 * @returns The Volume histogram series
 */
function applyVolume(series, chart, options = {}) {
    const colorUp = options.colorUp ?? '#26a69a';
    const colorDown = options.colorDown ?? '#ef5350';
    // Initialize cache for this series if it doesn't exist
    if (!volumeCache.has(series)) {
        volumeCache.set(series, new Map());
    }
    // Create histogram series in separate pane (paneIndex: 3)
    ...
    // Calculate Volume from series data + cache
    const calculateVolume = () => {
        ...
        for (const d of seriesData) {
            ...
            if (volume && 'close' in d && 'open' in d) {
                results.push({
                    time: d.time,
                    value: volume,
                    color: d.close >= d.open ? colorUp : colorDown,
                });
            }
        }
    ...
    const updateVolume = () => {
        try {
            ...
            console.error('Error calculating Volume:', error.stack);
        }
    };
    // Subscribe to data changes
    series.subscribeDataChanged(updateVolume);
    // Initial calculation
    updateVolume();
    return volumeSeries;
}
/**
 * Set volume data for a series
 * Call this function when you set data to your candlestick series
 * @param series - The candlestick series
 * @param data - Array with volume field
 * @example
 * const data = [{time: '2024-01-01', open: 100, high: 105, low: 99, close: 103, volume: 1000000}, ...];
 * candleSeries.setData(data);
 * setVolumeData(candleSeries, data); // Store volume for indicators
 */
function setVolumeData(series, data) {
    if (!volumeCache.has(series)) {
        volumeCache.set(series, new Map());
    }
    const cache = volumeCache.get(series);
...
const obvVolumeCache = new Map();
/**
 * Apply OBV (On-Balance Volume) indicator to a series
 * @param series - Source series
 * @param chart - Chart instance
 * @param options - OBV configuration
 * @returns The OBV line series
 */
function applyOBV(series, chart, options = {}) {
    const color = options.color ?? '#2196F3';
    // Initialize cache for this series if it doesn't exist
    if (!obvVolumeCache.has(series)) {
        obvVolumeCache.set(series, new Map());
    }
    // Create OBV line series in separate pane (paneIndex: 5)
    ...
    const calculateOBV = () => {
        const seriesData = series.data();
        const cache = obvVolumeCache.get(series);
        ...
        for (let i = 1; i < seriesData.length; i++) {
            ...
            const previous = seriesData[i - 1];
            const timeKey = typeof current.time === 'string' ? current.time : String(current.time);
            const volume = cache.get(timeKey);
            if (volume && 'close' in current && 'close' in previous) {
    ...
    // Subscribe to data changes
    series.subscribeDataChanged(updateOBV);
    // Initial calculation
    updateOBV();
    return obvSeries;
}
/**
 * Set volume data for OBV calculation
 * @param series - The candlestick series
 * @param data - Array with volume field
 */
function setOBVVolumeData(series, data) {
    if (!obvVolumeCache.has(series)) {
        obvVolumeCache.set(series, new Map());
    }
    const cache = obvVolumeCache.get(series);
...
/**
 * Draggable Price Line - Línea de precio que se puede arrastrar con el mouse
 */
class DraggablePriceLine {
    constructor(chart, series, options) {
        this._private__priceLine = null;
        ...
        this._private__createPriceLine();
        if (this._private__options.draggable) {
            this._private__setupDragHandlers();
        }
    }
    _private__createPriceLine() {
        ...
        this._private__priceLine = this._private__series.createPriceLine({
            ...
            lineWidth: this._private__options.lineWidth,
            lineStyle: this._private__options.lineStyle,
            axisLabelVisible: this._private__options.axisLabelVisible,
            title: this._private__options.title || '',
        });
    }
    _private__setupDragHandlers() {
        ...
        const handleMouseDown = (e) => {
            ...
            if (this._private__isOnLine(e, price)) {
                ...
                if (this._private__options.onDragStart) {
                    this._private__options.onDragStart(this._private__options.price);
                }
                e.preventDefault();
                e.stopPropagation();
            }
        ...
        const handleMouseMove = (e) => {
            if (this._private__isDragging) {
                const newPrice = this._private__getPriceFromY(e.clientY);
                this._private__updatePrice(newPrice);
    ...
    _private__updatePrice(newPrice) {
        ...
        if (this._private__priceLine) {
            this._private__priceLine.applyOptions({ price: newPrice });
        }
    }
    /**
     ...
     */
    setPrice(price) {
        this._private__updatePrice(price);
    }
    /**
     * Remove the price line and cleanup
     */
    remove() {
...
/**
 * Create a flexible trading line with presets for common trading scenarios
 * @param series - Series to attach the line to
 * @param chart - Chart instance
 * @param options - Trading line options
 * @returns DraggablePriceLine instance
 * @example
 * ```typescript
 * // Stop Loss
 * const sl = createTradingLine(candleSeries, chart, {
 *     price: 95.00,
 *     type: 'stop-loss',
 *     onDragEnd: (price) => console.log('New SL:', price)
 * });
 *
 * // Take Profit
 * const tp = createTradingLine(candleSeries, chart, {
 *     price: 105.00,
 *     type: 'take-profit'
 * });
 *
 * // Custom line
 * const custom = createTradingLine(candleSeries, chart, {
 *     price: 100.00,
 *     title: 'My Line',
 *     color: '#9C27B0',
 ...
 * ```
 */
function createTradingLine(series, chart, options) {
    // Presets for common trading line types (all draggable by default)
    const presets = {
        'stop-loss': {
            color: '#ef5350',
            lineStyle: 2 /* LineStyle.Dashed */,
        ...
        'entry': {
            ...
            lineStyle: 0 /* LineStyle.Solid */,
            title: 'Entry',
            draggable: true, // Changed to draggable
        },
        'target': {
            color: '#FF9800',
            lineStyle: 2 /* LineStyle.Dashed */,
    ...
    // Get preset or use custom defaults
    const preset = options.type ? presets[options.type] : presets['custom'];
    // Merge preset with user options (user options take priority)
    const lineOptions = {
        price: options.price,
        color: options.color ?? preset.color ?? '#9E9E9E',
        lineWidth: options.lineWidth ?? 2,
        lineStyle: options.lineStyle ?? preset.lineStyle ?? 0 /* LineStyle.Solid */,
        title: options.title ?? preset.title ?? '',
        axisLabelVisible: options.axisLabelVisible ?? true,
        draggable: options.draggable ?? preset.draggable ?? true,
        onDragStart: options.onDragStart,
        onDragMove: options.onDragMove,
        onDragEnd: options.onDragEnd,
    };
    return new DraggablePriceLine(chart, series, lineOptions);
}

/**
 * Interactive Line Manager - Permite crear líneas haciendo clic en el gráfico
 */
class InteractiveLineManager {
    constructor(chart, series) {
        this._private__container = null;
        this._private__creationMode = null;
        this._private__clickHandler = null;
        this._private__customOptions = {};
        this._private__chart = chart;
        this._private__series = series;
        this._private__setupContainer();
    }
    _private__setupContainer() {
        const chartElement = this._private__chart.chartElement?.() || document.querySelector('.tv-lightweight-charts');
        if (chartElement) {
            this._private__container = chartElement;
        }
     ...
     * @param mode - Tipo de línea a crear
     * @param customOptions - Opciones personalizadas para líneas custom
     * @returns Promesa que se resuelve cuando el usuario hace clic
     */
    enableClickToCreate(mode, customOptions) {
        if (!this._private__container) {
            return Promise.resolve(null);
        }
        this._private__creationMode = mode;
        this._private__customOptions = customOptions || {};
        return new Promise((resolve) => {
            // Change cursor
            this._private__container.style.cursor = 'crosshair';
            // Setup click handler
            this._private__clickHandler = (e) => {
                const price = this._private__getPriceFromMouseEvent(e);
                const line = this._private__createLineAtPrice(price);
                this.disableClickToCreate();
                resolve(line);
            };
            this._private__container.addEventListener('click', this._private__clickHandler);
        });
    }
    /**
     * Desactiva el modo de creación por clic
     */
    disableClickToCreate() {
        if (this._private__container && this._private__clickHandler) {
            this._private__container.removeEventListener('click', this._private__clickHandler);
            this._private__container.style.cursor = 'default';
        }
        this._private__creationMode = null;
        this._private__clickHandler = null;
    ...
    _private__createLineAtPrice(price) {
        if (!this._private__creationMode)
            return null;
        let options;
        switch (this._private__creationMode) {
            case 'stop-loss':
                options = {
                    price,
                    color: '#ef5350',
            ...
            case 'entry':
                options = {
                    ...
                    lineWidth: 2,
                    title: 'Entry',
                    draggable: true, // Changed to always draggable
                    ...this._private__customOptions,
                };
                break;
            case 'target':
        ...
        return new DraggablePriceLine(this._private__chart, this._private__series, options);
    }
    /**
     * Obtiene el modo actual de creación
     */
    getCreationMode() {
        return this._private__creationMode;
    }
    /**
     * Verifica si está en modo de creación
     */
    isCreating() {
        return this._private__creationMode !== null;
    }
}
/**
 * Crea un manager interactivo para añadir líneas con clics
 * @param chart - Chart instance
 * @param series - Series instance
 * @returns InteractiveLineManager
 * @example
 * ```typescript
 * const manager = createInteractiveLineManager(chart, candleSeries);
 *
 ...
 * ```
 */
function createInteractiveLineManager(chart, series) {
    return new InteractiveLineManager(chart, series);
}
...
// Note: Class-based indicators have been removed. Use the functional API above instead.
/**
